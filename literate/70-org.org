...
document; indenting also hides every star of a heading but the last. Editing inside folded text is refused rather than silently
mangling it. New headings go after the current subtree's content, =RET=
follows links, and =C-a=/=C-e= respect heading stars. Source blocks already
use their language's colours and =TAB= by default; here they also keep their
indentation exactly as written. Files open unfolded: Org 9.7
