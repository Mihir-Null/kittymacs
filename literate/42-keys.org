...
Windows are frames here: =SPC w n= makes a new OS window, and the lowercase
split keys split inside the current frame when you want that.
...
| =SPC n= notes, =SPC S= spelling | dropped | packages not in use; =SPC t s= toggles spelling |
| =SPC w h= / =SPC w v= new frame | =SPC w n= | lowercase splits are real splits now |
