...
parent, so every =C-x p= command works under =SPC p= as well, and the map
below only adds to it or overrides a key. The additions live here rather than
being written into Emacs's map, so =C-x p= stays exactly the menu the Emacs
manual describes. One key is overridden: =SPC p p= opens the project in its
own workspace, while =C-x p p= still switches project in the current one.
