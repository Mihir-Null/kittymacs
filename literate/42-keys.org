...
The project group is Emacs's own =project-prefix-map=, so =C-x p= and
=SPC p= are the same menu. Windows are frames here: =SPC w n= makes a new
OS window, and the lowercase split keys split inside the current frame
when you want that.
