...
capture checks the root and resolved target before writing. Safe nested capture
paths create missing parent directories after validating the nearest existing
parent; excluded and out-of-root parents are never created. Customize the
exclusion list with =setq= in =private.el= before this chapter loads.

* Native Windows physical paths

Windows graph operations use native CPython's standard-library
=os.path.realpath(path, strict=os.path.ALLOW_MISSING)=. This resolves NTFS
junctions, including missing capture suffixes; native Emacs's =file-truename=
alone does not do so. A maintained CPython with that capability must be on
=exec-path= as =python.exe=, or set an absolute trusted executable in =private.el=:

#+begin_example
(setq kittymacs-org-roam-python-executable "C:/path/to/Python/python.exe")
#+end_example

Capability is probed, not inferred from a version number (ALLOW_MISSING was
backported to several Python releases). Native CPython 3.14.7 was already
installed on the tested host; no installation, compiler or DLL is required
there. Cygwin Python, Tramp and UNC graphs are outside this local backend.
Ordinary startup and Org file visits still work without Python; graph commands
report the missing capability before opening a speculative database.

The adapter launches fixed code with =-I -S -B -u=, without a shell or project
imports. Each UTF-8 stdin line is a JSON array of absolute local paths; each
ASCII-safe stdout line is exactly ={"paths": [physical paths]}= or
={"error": "exception type: message"}=. This small stdlib contract is independent
of Org-roam; other applications can use it without loading kittymacs.
An operation owns at most one lazy pipe child and closes it on return, error or
quit, including its stderr pipe. Synchronous nested calls reuse that child;
reentry during a pending request fails clearly. Every request resolves anew,
and neither paths nor interpreter capabilities are cached across operations.
Malformed responses, process death and timeouts fail closed with stderr context.

Both lexical and physical excluded components are checked before descending.
The guarded walker visits each physical directory once, skips missing targets,
and returns canonical file names to upstream indexing. Root and DB placement,
direct upstream updates and capture targets use the same boundary. Capture
allows a dangling junction whose missing target is inside the graph, but only
after checking the nearest existing physical ancestor is a writable directory.
Missing roots, file-as-parent paths, escaped/excluded targets and resolution
errors are rejected. As with ordinary filename APIs, this is operation-time
validation, not an atomic defense against another process retargeting a link
between the check and a subsequent filesystem operation.

The lasting native fixture is =tests/org-roam-windows-fixture.ps1= (PowerShell
7). Supply a new task-owned =-FixtureRoot= plus absolute =-Emacs=, =-Python= and
=-Packages= paths. It retains real NTFS junctions for inspection, including
escape, excluded-target, alias, dangling and cycle cases; it never recursively
cleans up junction trees. The retarget test only unlinks its explicitly checked
=retarget= junction and leaves targets intact. Batch helper fault injections
cover protocol/death/timeout behavior. Real denied ACLs, UNC shares, mounted
volumes and long-path limits need separate host acceptance.
