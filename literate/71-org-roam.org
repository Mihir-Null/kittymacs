...
compute the database path. A graph's physical identity is =file-truename=,
lower-cased on a case-insensitive file system, so symbolic links to one
graph share that path and separate worktrees have separate paths. An alias
that =file-truename= cannot resolve, such as an NTFS junction on native
Windows, is a different path and gets its own cache. Databases belong under the user cache, outside
...
The default exclusions are the folders a personal notes folder usually
holds but that are not notes: =.git/=, =.cache/=, =cache/= and =caches/=.
Symlinks leading out of the graph are excluded.
