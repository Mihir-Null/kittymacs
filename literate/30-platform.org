...
On Linux and macOS the import also brings the Nix variables along.

This is arranged once, when the module loads, rather than inside
=kittymacs-platform-apply= below: =with-eval-after-load= registers its body
permanently each time it runs, so a function that calls it would queue
another copy on every call. Nothing here depends on =private.el=, so waiting
...
Shell discovery uses available executables. Windows also borrows Git for Windows' Unix tools, last on =exec-path=: =diff=, =diff3= and =patch= for Magit's hunk refinement, Ediff and diff-hl.
...
The rest of the shell choice, the Unix tools on Windows, and the hand-off to the macOS and Android sections.
...
The Keychain is the one part that has to wait for a library, and it waits
outside the policy function. =with-eval-after-load= registers its body
permanently, once per call, so calling it from a function that may run more
than once (the tests call =kittymacs-platform-apply= dozens of times) would
queue the same work again on every call. Registered once when the module
loads, it asks which platform it is on when =auth-source= arrives.
...
The hooks that turn Flyspell on are added unconditionally, and each one asks
for a checker only when a buffer opens. Deciding once, while this module
loads, would be too early: the module loads before =private.el=, so a
=kittymacs-msys2-root= set there, or a checker that =exec-path-from-shell= later
finds on the login shell's =PATH=, would never turn spelling on. The question
costs two lookups on =exec-path= per buffer, which is nothing next to what
Flyspell itself does.
