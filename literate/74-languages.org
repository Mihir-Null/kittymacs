...
The configuration checkout stays source-only. Compiled parser libraries belong under the disposable kittymacs cache, and the directory is added before any grammar availability checks. Set this option in =private.el= to use another writable location.
