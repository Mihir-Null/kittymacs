...
(ert-deftest kittymacs-treesit-installer-forwards-pin-and-destination ()
  "Installing one grammar passes the pinned recipe and writable destination."
  (let ((kittymacs-treesit-grammar-directory "/tmp/kittymacs-grammars/")
        (treesit-language-source-alist nil)
        installed)
    (cl-letf (((symbol-function 'treesit-available-p) (lambda () t))
              ((symbol-function 'treesit-install-language-grammar)
               (lambda (&rest arguments) (setq installed arguments)))
              ((symbol-function 'kittymacs-treesit-refresh-mode-remaps)
               #'ignore))
      (kittymacs-treesit-install-language-grammar 'nix))
    (should (equal installed '(nix "/tmp/kittymacs-grammars/")))
...
(ert-deftest kittymacs-treesit-install-all-uses-one-destination ()
  "Bulk maintenance installs every missing grammar into the same cache."
  (let ((kittymacs-treesit-grammar-directory "/tmp/kittymacs-grammars/")
    ...
    (cl-letf (((symbol-function 'treesit-available-p) (lambda () t))
              ((symbol-function 'kittymacs-treesit--language-available-p)
               (lambda (_) nil))
              ((symbol-function 'treesit-install-language-grammar)
               (lambda (&rest arguments) (push arguments installed)))
              ((symbol-function 'kittymacs-treesit-refresh-mode-remaps)
               #'ignore))
      (kittymacs-treesit-install-all-grammars))
    (should (equal (nreverse installed)
                   '((bash "/tmp/kittymacs-grammars/")
                     (nix "/tmp/kittymacs-grammars/"))))))
